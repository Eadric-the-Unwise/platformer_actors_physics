...
void check_J(UINT8 newplayerx, UINT8 newplayery, INT16 camera_x)
{
    ...
    if (!LADDER)
    { // RESET LADDER RELEASE IF JUMPING FROM THE GROUND TO ALLOW GRABBING WHEN SPDY = 0
        if (LADDER_Release)
        {
    ...
    else if (!CROUCH)
    {
        ...
        { // IF WALK SPEED MAX, THEN ALLOW SOME LEEWAY ON WHEN HE IS ABLE TO JUMP OUT OF A CORNER (EXCEPT INTO THE CORNER FROM OUTSIDE)
...
void check_world_UD(UINT8 newplayerx, UINT8 newplayery, INT16 camera_x, INT16 camera_y)
{
    ...
    if ((COLLISION_DATA[tileindexL] == 0x01) || (COLLISION_DATA[tileindexR] == 0x01 || y_Collide))
    {
        PLAYER.SpdY = 0;
    }
