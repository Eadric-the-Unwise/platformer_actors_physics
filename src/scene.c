...
void load_bullets(const actor_t *bullet, UINT8 bullet_count, UINT8 hiwater)
    ...
    for (UINT8 i = bullet_count; i != 0; i--) {
        ...
        current_bullet->x_offset = bullet->x_offset;
        current_bullet->y_offset = bullet->y_offset;
...
UINT8 reload_NPC_actors(const actor_t *actor, UINT8 actors_count)
    ...
    for (UINT8 i = 4; i != 0; i--) {  // counter direction does not matter, because pointer is moved. only number of iterations matter.
        current_actor->bank = actor->bank;
        current_actor->bullet = actor->bullet;
        current_actor->copy = actor->copy;
        current_actor->tile_count = actor->tile_count;
...
void switch_ladder() {
    ...
    if (PLAYER.direction == DIR_LADDER_L && PLAYER.patrol_timer == 0) {
        SetActorDirection(&PLAYER, DIR_LADDER_R, 0);
        PLAYER.patrol_timer = PLAYER.patrol_max;
...
void NPC_reset(UINT8 actor_count) {
    ...
    reset_actor++;
    for (UINT8 x = actor_count - 1; x != 0; x--) {  // TURN OFF ALL NPC SPRITES
        // reset_actor->x = reset_actor->x_reset;
        // reset_actor->y = reset_actor->y_reset;
        // reset_actor->patrol_timer = reset_actor->patrol_reset;
        reset_actor->RENDER = FALSE;
        reset_actor->ON = FALSE;
        reset_actor++;
    }
}
